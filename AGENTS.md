...
| `src/roster.js` | Roster = the SHARED v1 file `~/.consensflow/agents.json` (cc + pi read/write it too): v1-schema-faithful mapping (kind↔runtime, thinking/effort↔effort, toolsPolicy↔permission), unknown fields preserved, unsupported kinds listed+marked, never dropped. **`CONSENSFLOW_HOME` means one root to both halves**: set it and the roster, the state and the workspaces all live under it; unset, the roster stays `~/.consensflow` and state goes to XDG `~/.config/consensflow`. It used to mean the state root here and the roster root in the payload, which split a machine in two |
