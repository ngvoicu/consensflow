...
`app/` is a Tauri shell around the same editor — a real macOS `.app` with a
Dock icon, built from this repo:

```sh
cd app && npm install && npx tauri build
# → app/src-tauri/target/release/bundle/macos/ConsensFlow.app (and a .dmg)
```

It is a window, not a second implementation: on launch it locates the
`consensflow` CLI (PATH, then the usual install locations, then your login
shell — a .app from Finder inherits none of your shell's PATH), starts
`consensflow ui --json --no-open` as a child, and points the window at it.
The editor exits when the app does, because the app holds a pipe to its
stdin. So the app needs the CLI installed, and it always shows the same
editor `cf ui` does.
