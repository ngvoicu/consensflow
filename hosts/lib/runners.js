...
export function buildRunnerInvocation(agent, packetPath, cwd, session) {
  ...
  switch (p.kind) {
    ...
    case "claude-code": {
      // Deliberately NO --bare (removed in 1.5.1): on Claude Code >= 2.1 --bare means "Anthropic
      // auth is strictly ANTHROPIC_API_KEY or apiKeyHelper — OAuth and keychain are never read",
      // and we dropEnv ANTHROPIC_API_KEY below so runs ride the subscription login. The two
      // together left claude children with NO auth path at all ("Not logged in · Please run
      // /login"). Recursion is fully guarded by CONSENSFLOW_CHILD instead: both hook scripts and
      // `cf run` bail under it, so a child that loads this plugin does nothing with it — parity
      // with pi children, which never ran --bare.
