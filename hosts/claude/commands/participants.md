...
```bash
node "${CONSENSFLOW_HOST_ROOT}/bin/cf.mjs" agents $ARGUMENTS
```

With no arguments this lists the configured agents. Keep flags and single words as-is; shell-quote any argument containing spaces (e.g. a `--description` value).
