...
import { buildRunnerInvocation, normalizeProcessOutput } from '../../hosts/lib/runners.js'
import {
  adaptLine,
  MAX_EVENT_CHARS,
  MAX_EVENTS,
...
test('transcript-events: OpenCode adapter maps text/tool/thinking, skips unknowns, bounds the trail [STRM-05]', async () => {
  ...
  assert.ok(
    texts.some((e) => /continue from where the lead left off/.test(e.text)),
    'keeps the substantive text',
  )
  ...
  assert.ok(
    results.length >= 1 && typeof results[0].result === 'string',
    'tool_result carries the output string',
  )
  ...
  const thinking = adaptLine('opencode', {
    ...
    part: {
      state: {
        metadata: {
          openrouter: { reasoning_details: [{ type: 'reasoning.text', text: 'let me think' }] },
        },
      },
    },
  })
  ...
  const big = adaptLine('opencode', {
    type: 'text',
    part: { text: 'z'.repeat(MAX_EVENT_CHARS + 5000) },
  })
  ...
  pushEvents(
    trail,
    Array.from({ length: MAX_EVENTS + 50 }, () => ({ kind: 'text', text: 'x' })),
  )
...
const codexFixture = async () => {
  const raw = await readFile(
    new URL('./fixtures/codex-exec-json.sample.jsonl', import.meta.url),
    'utf8',
  )
  return raw
    .split(/\r?\n/)
...
test('transcript-events: codex adapter maps exec tool calls, harness messages, and reasoning [STRM-09]', async () => {
  ...
  assert.deepEqual(
    adaptLine('codex', {
      type: 'item.completed',
      item: { type: 'reasoning', text: 'let me reason' },
    }),
...
const piFixture = async () => {
  const raw = await readFile(
    new URL('./fixtures/pi-mode-json.sample.jsonl', import.meta.url),
    'utf8',
  )
  return raw
    .split(/\r?\n/)
...
test('transcript-events: pi adapter maps tool_execution + assistant message_end (no end-summary dupes) [STRM-07]', async () => {
  ...
  const results = events.filter((e) => e.kind === 'tool_result')
  assert.ok(
    calls.some((c) => c.tool === 'read' && typeof c.args?.path === 'string'),
    'read tool_call with args',
  )
  assert.ok(
    results.some((r) => /hello world from the consensflow fixture/.test(r.result)),
    'tool_result carries the read output',
  )

  // Assistant message_end text → text; the final one-word answer surfaces.
  const texts = events.filter((e) => e.kind === 'text')
  assert.ok(
    texts.some((e) => /hello/i.test(e.text)),
    'assistant text surfaces',
  )
  // user / toolResult message_end roles are skipped — the tool output must not leak in as "text".
  assert.ok(
    !texts.some((e) => /2 more lines/.test(e.text)),
    'tool output never leaks into a text event',
  )
  ...
  assert.deepEqual(
    adaptLine('pi', {
      type: 'message_end',
      message: { role: 'assistant', content: [{ type: 'reasoning', text: 'hmm' }] },
    }),
...
const claudeStreamFixture = async () =>
  (await readFile(new URL('./fixtures/claude-stream-json.sample.jsonl', import.meta.url), 'utf8'))
    .split(/\r?\n/)
    .filter(Boolean)
...
test('transcript-events: claude adapter (stream-json content blocks) + final extraction + invocation [STRM-11]', async () => {
  ...
  assert.ok(
    ...
    'thinking block surfaces',
  )
  const calls = events.filter((e) => e.kind === 'tool_call')
  assert.ok(
    calls.some((c) => c.tool === 'Read' && c.args),
    'tool_use → tool_call (Read, with args)',
  )
  assert.ok(
    events.some(
      (e) => e.kind === 'tool_result' && /hello world from the consensflow fixture/.test(e.result),
    ),
    'tool_result content surfaces',
  )
  assert.ok(
    events.some((e) => e.kind === 'text' && /hello/i.test(e.text)),
    'final text block surfaces',
  )
  ...
  assert.equal(
    normalizeProcessOutput('claude-code', JSON.stringify({ type: 'result', result: 'SOLO' }), '')
      .output,
    'SOLO',
  )
  ...
  const inv = buildRunnerInvocation(
    { kind: 'claude-code', toolsPolicy: 'workspace-write', model: 'claude-opus-4-8' },
    '/tmp/p.md',
    '/repo',
  )
